...
fn ssr_renders_div_and_canvas() {
    ...
    assert!(
        ...
        "missing wrapper class, got: {html}"
    );
    assert!(
        html.contains("<canvas"),
        "missing canvas element, got: {html}"
    );
    assert!(
        html.contains("width=\"320\""),
        "canvas width should match prop, got: {html}"
    );
    assert!(
        html.contains("height=\"200\""),
        "canvas height should match prop, got: {html}"
    );
}
...
fn ssr_renders_foreground_children() {
    ...
    assert!(
        html.contains("hello from inside"),
        "child text should appear, got: {html}"
    );
}
