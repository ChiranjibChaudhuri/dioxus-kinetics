...
fn two_regions_both_render_without_overwriting() {
    ...
    assert!(
        pixels[p1] > 50,
        "expected red tint in region 1 (center 32,32), got R={}",
        pixels[p1]
    );
    let p2 = ((96 * 128 + 96) * 4) as usize;
    assert!(
        pixels[p2 + 1] > 50,
        "expected green tint in region 2 (center 96,96), got G={}",
        pixels[p2 + 1]
    );
    let corner_idx = 0;
...
fn overlapping_regions_follow_last_wins() {
    ...
    assert!(
        g_overlap > r_overlap,
        "overlap should be green-dominant (upper wins): G={g_overlap}, R={r_overlap}"
    );
    ...
    assert!(
        r_lower > g_lower,
        "lower-only zone should be red-dominant: R={r_lower}, G={g_lower}"
    );
}
