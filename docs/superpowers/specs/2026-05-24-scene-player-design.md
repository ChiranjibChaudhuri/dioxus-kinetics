# Scene Player — Design (SP-1)

## Goal

Land the keystone sub-project of the "HyperFrames for Dioxus" track: a
single paused-seekable **`Scene`** Dioxus component that owns one
...
`Scene` gallery category that subsequent sub-projects expand.

> Naming note. The data struct `ui_composition::Composition`
> (id/width/height/fps/frame_count) already exists. The new Dioxus
> component is therefore named **`Scene`**, not `Composition`, to avoid
> the type collision. The component sits in a new gallery category also
> named **`Scene`**. The cinematic intent (a scene with timed beats,
> transport controls, multiple animation runtimes coordinated by one
> clock) is what this spec inherits from HeyGen's `hyperframes` work.
> Internal modules and types use the `scene_*` / `Scene*` naming
> throughout.
...
1. A `FrameAdapter` trait in `ui-runtime::frame_adapter` plus three
   ...
   `CssKeyframesAdapter`).
2. A signal-backed `SceneClock` in `ui-runtime::scene_clock`
...
3. A `Scene` Dioxus component in `ui-dioxus::scene_player`
   that hosts the clock, provides a `SceneContext` via Dioxus
...
4. A `Clip` Dioxus component that consumes the parent
   `SceneContext` and actually shows/hides + fades its children
...
6. A new gallery category `Scene` (slug `scene`) and one
   cinematic showcase entry, **Scene · Product Intro 10s**, that
   exercises every piece end-to-end and is scrubbable in the gallery.
7. A Playwright spec under
   `examples/component-gallery/e2e/tests/scene-player.spec.ts` that
   scrubs the showcase from 0 % to 100 % and verifies settled end state.
8. Public API re-exports from `kinetics::prelude` (`Scene`, `Clip`,
   `SceneState`, `FrameAdapter`, `SequenceAdapter`, `WaapiAdapter`,
...
crates/ui-runtime/src/
  scene_clock.rs    # signal-backed transport state
...
crates/ui-dioxus/src/
  scene_player.rs   # Scene + Clip + scrubber UI
  lib.rs                 # re-exports

crates/ui-styles/src/
  scene_player.css  # scrubber, transport bar, settled tag

examples/component-gallery/src/previews/
  scene.rs                     # gallery preview module
  scenes/
    mod.rs
...
`SceneClock` is `Copy` because every field is a Dioxus `Signal<T>`
...
pub struct SceneContext {
    ...
    pub fps: u32,
    pub duration_ms: f32,
    pub clock: SceneClock,                  // exposes signals + transport
...
When `controls: true`, a `<div class="ui-scene-transport">` is
rendered after children with a play/pause `<button>`, a
`<input type="range" min=0 max=duration_ms>` scrubber, and a
`<span class="ui-scene-time">` time readout.

`Scene` is responsible for:

- Constructing the `SceneClock` on first render.
- Providing the `SceneContext` via context.
...
Subscribes to `SceneContext.clock.elapsed_ms` and computes
...
[clock.seek_ms(ms)]
    ↓ Signal<f32>::set
[SceneContext.clock.elapsed_ms updates]
    ↓ use_effect in Scene (deps: elapsed_ms + state)
...
[non-adapter Sequence / KineticBox children re-sample via
 ...
 to SceneContext.clock.elapsed_ms through use_timeline_sample]
```
...
gains a "Legacy data-attr stage" note linking to the Scene
category.
...
- Add `ComponentCategory::Scene` after `ComponentCategory::Capture`.
- Label: `Scene`. Slug: `scene`. Description:
...
- One `ComponentDoc` entry,
  ...
  inlined from `previews/scenes/product_intro.rs`.
- The `populated_categories()` filter in `app.rs` will automatically
  pick up the new category once the entry lands.

`examples/component-gallery/src/previews/scenes/product_intro.rs`:
...
Wrapped in a `Scene { controls: true }` so reviewers can scrub.
...
sibling files under `scenes/` to keep `product_intro.rs`
...
- `scene_clock::tests::play_advances_elapsed` — under a fake
  rAF source, `elapsed_ms` reaches `duration_ms` and transitions to
  `Settled`.
- `scene_clock::tests::seek_clamps` — `seek_ms(-50.0)` clamps
  to 0; `seek_ms(duration_ms + 100.0)` clamps to `duration_ms` and
  transitions to `Settled`.
- `scene_clock::tests::reduced_freezes_at_settled` —
...
- `scene-player.spec.ts`:
  1. Visit `/component-gallery/#scene`.
  2. Find the `Scene · Product Intro 10s` card.
  ...
  5. Toggle the gallery's Motion preference to `Reduced`; assert
     ...
     `.ui-scene-reduced-tag` is visible.
...
one snapshot: `scene-player--settled.png`. We do **not** add
...
- `crates/ui-runtime/src/scene_clock.rs`
...
- `crates/ui-dioxus/src/scene_player.rs`
- `crates/ui-styles/src/scene_player.css`
- `examples/component-gallery/src/previews/scene.rs`
- `examples/component-gallery/src/previews/scenes/mod.rs`
- `examples/component-gallery/src/previews/scenes/product_intro.rs`
- `examples/component-gallery/src/previews/scenes/flip_card_deck.rs`
- `examples/component-gallery/src/previews/scenes/metric_counter.rs`
- `examples/component-gallery/src/previews/scenes/cta_pulse.rs`
- `examples/component-gallery/e2e/tests/scene-player.spec.ts`

Edited:

- `crates/ui-runtime/src/lib.rs` — re-export new modules.
- `crates/ui-dioxus/src/lib.rs` — re-export `Scene`, `Clip`,
  `SceneState`.
...
| Adapter registry over-renders when child adapter components mount / unmount frequently (e.g. inside a `for` loop). | Registry is `Rc<RefCell<…>>` with stable id keys; effect runs only on mount and on `adapter.id()` change. The `Scene` effect that fans out `seek` reads the registry once per frame, not per registration event. |
...
| The new Scene gallery entry slows the gallery's first paint. | `autoplay` defaults to `true` only inside the dedicated showcase; the gallery preview wraps it in the existing `ReplayFrame` / `ScrubFrame` analog so it's offscreen until scrolled into view via `IntersectionObserver` (already used by `ScrubFrame`). |
...
2. **Time is `f32` milliseconds, not `u32` frames.** HyperFrames uses
   ...
   work.
3. **`Scene` is *not* a renderer.** It is a player. The render
...
- The Playwright spec in `e2e/tests/scene-player.spec.ts` passes
  in CI on Chromium and WebKit.
- The gallery renders a new `Scene` section between `Capture`
  and the end of the page (or wherever `populated_categories()`
  surfaces it given category order in `docs.rs`).
- Scrubbing the `Scene · Product Intro 10s` entry from 0 % to
...
- **SP-2:** Cinematic showcase library — port four more scenes
  ...
  to the same `Scene` API. Decide whether to ship as gallery
...
- **SP-3:** GSAP-tier primitives — `ScrollTrigger` (a `FrameAdapter`
  ...
  rotate cue stream).
- **SP-4:** Headless render pipeline — drive `Scene` frame by
